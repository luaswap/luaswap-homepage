...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should not allow non-owner to do operation', async () => {
        await this.lua.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.lua.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.lua.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.lua.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.lua.owner()).valueOf(), carol);
    });


    it('should also work with LuaMasterFarmer', async () => {
    ...
    it('should cancel tx', async () => {
        ...
        assert.equal((await this.lua.owner()).valueOf(), this.timelock.address);
    });
});